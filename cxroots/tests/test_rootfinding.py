...
class MultiplicityTests(object):
	def test_multiplicity_f(self):
		...
		for i, root in enumerate(self.roots):
			assert get_multiplicity(self.f, root, df=None, contour=self.C, verbose=True) == self.multiplicities[i]

	def test_multiplicity_df(self):
		# Check that if only the root is given then the multiplcity could be computed
		for i, root in enumerate(self.roots):
			assert get_multiplicity(self.f, root, df=self.df, contour=self.C, verbose=True) == self.multiplicities[i]
